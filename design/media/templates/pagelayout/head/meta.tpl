...
<link rel="canonical" href="{$mt_meta.canonical_url|wash}" />
    ...
    {if $mt_meta.og_image|gt(0)}
        ...
        {if and($mt_image_object, is_set($mt_image_object.data_map))}
            ...
            {if and(not($mt_image_attr), $mt_image_object.data_map.site_logo)}{set $mt_image_attr = $mt_image_object.data_map.site_logo}{/if}
            {if and(not($mt_image_attr), $mt_image_object.data_map.image)}{set $mt_image_attr = $mt_image_object.data_map.image}{/if}
            {if and(not($mt_image_attr), $mt_image_object.data_map.file)}{set $mt_image_attr = $mt_image_object.data_map.file}{/if}
            {if $mt_image_attr}
                {def $mt_image_path = ''}
                {def $mt_image_width = ''}
                {def $mt_image_height = ''}
